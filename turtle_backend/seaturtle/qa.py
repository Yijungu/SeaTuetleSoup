...
def question(query):
    query = "질문에 대해 '네' '아니오' 로만 짧게 대답해줘 그리고 모르겠으면 추측해서 대답하고 그래도 모르겠으면 모르겠다고 해. " + query 
    print(query)
...
def submit(answer):
    messages = []
    answer += " 이 글에서 관측, 낚시대, 산이랑 비슷한 단어가 다 들어가있어? 길게 설명하지 말고 네 아니오로만 대답해줘"
...
def answer_plus_edit():
    ...
    global answer_plus
    answer_plus = " 이 글에서 " + keywords_str + "이랑 비슷한 단어가 다 들어가 있어? 길게 설명하지 말고 네 아니오로만 대답해줘"
